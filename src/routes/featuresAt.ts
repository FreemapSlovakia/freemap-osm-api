...
import { z } from 'zod';
import { getKeySets, queryJson } from '../db.js';
...
function parseKeys(keys: string): string[] {
  ...
  const unknown = list.filter((key) => !indexed.has(key));

  if (unknown.length > 0) {
    throw new FilterError(`tag keys are not searchable: ${unknown.join(', ')}`);
  }
